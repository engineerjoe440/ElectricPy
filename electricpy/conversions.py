...
def sequencez(Zabc, reference='A', resolve=False, diag=False, rounds=3):
    ...
    # Use Aabc/A012 consistently (Aabc maps ABC->012, A012 maps 012->ABC)
    # For different phase reference selections, roll the transformation matrix rows.
    Mabc = _np.roll(Aabc, roll, 0)   # ABC -> 012 (with reference shift)
    M012 = _np.roll(A012, roll, 0)   # 012 -> ABC (with reference shift)

    # Compute Sequence Impedance
    # Standard: Z012 = Aabc * Zabc * A012
    Z012 = Mabc.dot(Zabc.dot(M012))
