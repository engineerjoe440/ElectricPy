...
def busvolt(k, n, Vpf, Z0, Z1, Z2, If, sequence=True, reference="A"):
    ...
    Zmat = _np.array([[Z0[k, n], 0, 0], [0, Z1[k, n], 0], [0, 0, Z2[k, n]]])
    # Perform Calculation
...
def pickup(Iloadmax, Ifaultmin, scale=0, printout=False, units="A"):
    ...
    setpoint = _np.ceil(IL2 * 10 ** (-exponent + 1 + scale)) * 10 ** (
        exponent - 1 - scale
    )
    if printout:
...
def correctedcurrents(Ipri, TAP, correction="Y", CTR=1):
    ...
    MAT = {"Y": XFMY0, "D+": XFMD1, "D-": XFMD11, "Z": XFM12}
    # Condition Inputs
...
def distmeasz(
    VLNmeas, If, Ip, Ipp, CTR=1, VTR=1, k0=None, z1=None, z0=None, linelength=1
):
    """
