...
def digifiltersim(
    ...
    for i in range(flen):
        ...
        if sz < 5:
            raise ValueError(
                "ERROR: Too few filter arguments provided. "
...
def ramp_response(
    ...
    xlim=False,
    title="Ramp Response",
    errtitle="Ramp Response Error",
    resplabel="Ramp Response",
    funclabel="Ramp Function",
    errlabel="Error",
    filename=None,
):
    """
...
def parabolic_response(
    ...
    errtitle="Parabolic Response Error",
    resplabel="Parabolic Response",
    funclabel="Parabolic Function",
    errlabel="Error",
    filename=None,
):
    """
...
def statespace(
    ...
    if simpts >= NN:
        _warn(
            "WARNING: NN must be greater than simpts; NN="
            + str(NN)
            + "simpts="
            + str(simpts),
            " Autocorrecting simpts to be NN-1.",
        )
        simpts = NN - 1
    ...
    if cA != rA:  # A isn't nxn matrix
        ...
        (solution == 3) and (cC != cA) or (rC != 1)
    ):  # Number of elements in C don't meet requirements
        raise ValueError(
            "'C' matrix dimensions don't match state-space variable dimensions."
        )
    elif (solution == 3) and (
        (cD != rF) or (rD != 1)
    ):  # Number of elements in D don't meet requirements
        if (cD == rD) and (cD == 1) and (D[0] == 0):  # D matrix is set to [0]
            D = _np.asmatrix(_np.zeros(rF))  # Re-create D to meet requirements
            _warn(
                "WARNING: Autogenerating 'D' matrix of zeros to match forcing functions."
            )
        else:
            raise ValueError(
                "'D' matrix dimensions don't match forcing function dimensions."
            )
...
def NewtonRaphson(F, J, X0, eps=1e-4, mxiter=100, lsq_eps=0.25):
    ...
    if f0sz != j0sz:  # Size mismatch
        raise ValueError(
            "ERROR: The arguments return arrays or lists"
            + " of different sizes: f0="
            + str(f0sz)
            + "; j0="
            + str(j0sz)
        )
