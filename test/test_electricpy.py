...
def test_suspension_insulators():
    ...
    _, string_efficiency = suspension_insulators(
        number_capacitors, capacitance_ration, Voltage
    )
...
def test_abc_to_seq():
    ...
    def test_0():
        ...
        np.testing.assert_array_almost_equal(
            abc_to_seq([0, 1, 0]), [1 / 3 + 0j, a / 3 + 0j, a * a / 3 + 0j]
        )
        np.testing.assert_array_almost_equal(
            abc_to_seq([0, 0, 1]), [1 / 3 + 0j, a * a / 3, a / 3]
        )
...
class Test_air_core_inductor:
    def invoke(test_case):
        def wrapper(self):
            ...
            computed_result = air_core_inductor(
                self.coil_diameter, self.coil_length, self.turn
            )
